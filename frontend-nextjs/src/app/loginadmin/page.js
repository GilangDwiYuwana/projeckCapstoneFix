...
export default function LoginAdminPage() {
  const { login } = useAuth();
  const [email, setEmail]       = useState('owner@honda.com');
  ...
  const handleSubmit = async (event) => {
    ...
    setError('');
    setLoading(true);
    const success = await login({ email, password, role: 'owner' });
    if (success) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <div className="w-full max-w-2xl rounded-4xl border border-slate-200 bg-white p-8 shadow-xl">
        <div className="mb-6 flex items-center gap-3">
          <div className="rounded-2xl bg-red-50 p-3 text-red-600"><ShieldCheck className="h-6 w-6" /></div>
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-red-500">Login Admin</p>
            <h1 className="text-2xl font-bold text-slate-900">Masuk sebagai Owner / Admin</h1>
          </div>
        </div>
        ...
        <a href="/loginstaff" className="mt-4 inline-block text-sm font-semibold text-red-600">Login sebagai Staff</a>
      </div>
    </div>
  );
}